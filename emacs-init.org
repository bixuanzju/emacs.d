...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package haskell-mode
    ...
    (defun my/stack-mode-goto ()
      "if `stack-mode-goto' cannot find the definition, delegate to
  `haskell-mode-jump-to-def-or-tag'."
      (interactive)
      (condition-case nil
          (stack-mode-goto)
        (error (haskell-mode-jump-to-def-or-tag))))

    (require 'haskell)
    (bind-keys :map interactive-haskell-mode-map
               ("M-." . my/stack-mode-goto)
               ("M-," . xref-pop-marker-stack)
               ("C-c C-b" . stack-mode-type)
               ("C-c C-i" . stack-mode-info)
...
#+begin_src emacs-lisp :tangle yes
  (use-package avy
    ...
    (bind-key* "C-'" 'avy-goto-word-or-subword-1))
#+end_src
